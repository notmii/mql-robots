...
void computeIndicators()
{
    ...
    diff = (int)(MathAbs(lastOpenPrice - Bid) / Point);
}
...
void OnTick()
{
    ...
    Comment(
        ...
        "\n    Diff: ", diff,
        "\n    Avg True Range: ", (int)(avgTrueRange_D1 / Point),
        "\n    True Range: ", (int)(trueRange_D1 / Point),
        ""
    );

    if (diff >= ((int)(trueRange_D1 / Point)) || OrdersTotal() == 0) {
        int ticketNumber;
        ...
        if (ticketNumber > 0) {
            ...
            OrderModify(
                ticketNumber,
                OrderOpenPrice(),
                OrderOpenPrice() - avgTrueRange_D1 * 3,
                OrderOpenPrice() + trueRange_D1,
                0,
                Green
            );
        }

        ticketNumber = OrderSend(NULL, OP_SELL, tradeLots, Bid, 3, 0, 0, "Buy Test", 0, 0, Green);
        if (ticketNumber > 0) {
            OrderSelect(ticketNumber, SELECT_BY_TICKET);
            OrderModify(
                ticketNumber,
    ...
    if (AccountEquity() > initialAccountBalance + 100) {
        while(OrdersTotal() > 0) {
